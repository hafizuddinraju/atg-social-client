...
const RightSideContent = () => {
    ...
    const handleClick =(id)=>{
        ...
        const filterData = groupItem.filter(data => data.id === id);
        ...
        <div className='col-md-10'>
            {
                loggedIn ?
                    <>
...

                            {
                                groupItem?.map(group => <div key={group.id} group={group} className='mt-4'>
                                    <div className='d-flex justify-content-between align-items-center'>
                                        ...
                                        <div>
                                    {
                                        value[0] && value[0].id === group.id?
                                        <button onClick={()=> handleClick(group.id)} className='border-0 px-3 rounded-pill ' style={{backgroundColor: 'black', color:'white'}}>Followed</button>
                                        :
                                        <button onClick={()=> handleClick(group.id)} className='border-0 px-3 rounded-pill'>Follow</button>
                        ...
                        <p className='text-[12px] text-primary mt-4 text-end'>see more...</p>
                    </>
                    :
                    <>
                        <div className='d-flex justify-content-between border-bottom border-1 py-2'>
